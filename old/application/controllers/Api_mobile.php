...
class Api_mobile extends CI_Controller {
    ...
    {
        $main_image = !empty($property->main_image) ? base_url($property->main_image) : base_url('assets/img/home-3/property/property-09.jpg');
    ...
    /**
     * Format notification object to array with full URLs
     */
    private function _format_notification($notification)
    {
        ...
        return array(
            ...
            'updated_at' => isset($notification->updated_at) ? $notification->updated_at : null
        );
    }
    ...
    public function properties()
    {
        try {
            ...
            unset($total_filters['limit'], $total_filters['offset']);
            $total = count($this->Property_model->search($total_filters));
    ...
    public function categories()
    {
        try {
            ...
            if ($with_count) {
                $categories = $this->Category_model->get_all_with_property_count($status, $city, $location);
            } else {
    ...
    public function locations()
    {
        try {
            ...
            if ($with_count) {
                $locations = $this->Location_model->get_all_with_property_count($status, $city_id);
            } else {
    ...
    public function enquiry()
    {
        try {
            ...
            $user_id = $this->_get_input('user_id');
            $data = array(
     ...
     * GET /api/mobile/enquiries/user/{user_id}
     * Query params: status (optional) - filter by enquiry status (new, read, replied)
     */
    ...
    public function home()
    {
        try {
            ...
            $categories = $this->Category_model->get_all_with_property_count('active', $city, $location);
            ...
            $locations = $this->Location_model->get_all_with_property_count('active', $city_id);
    ...
    public function send_otp()
    {
        try {
            ...
            $development_mode = false; // Force development mode for now
    ...
    public function verify_otp()
    {
        try {
            ...
            if (is_array($result) && isset($result['success']) && $result['success']) {
                ...
                $needs_profile = empty($user->fullname) || empty($user->email);
                ...
                $this->_send_response(true, array(
                    ...
                    'user' => array(
                        ...
                        'profilepic'   => !empty($user->profilepic) ? base_url($user->profilepic) : null,
                        'token'        => $user->id
                    )
    ...
    public function save_profile()
    {
        try {
            $input    = $this->_get_input();
            $phone    = isset($input['phone'])        ? $input['phone']        : $this->input->post('phone');
            $cc       = isset($input['country_code']) ? $input['country_code'] : ($this->input->post('country_code') ?: '+91');
            $fullname = isset($input['fullname'])     ? $input['fullname']     : $this->input->post('fullname');
            $email    = isset($input['email'])        ? $input['email']        : $this->input->post('email');
            $city     = isset($input['city'])         ? $input['city']         : $this->input->post('city');
            $state    = isset($input['state'])        ? $input['state']        : $this->input->post('state');
            $pincode  = isset($input['pincode'])      ? $input['pincode']      : $this->input->post('pincode');

            // Identify user by session OR by phone (for first-time profile save after OTP)
            $user_id = $this->session->userdata('user_id');
            if (empty($user_id) && !empty($phone)) {
                $user = $this->User_model->get_by_phone($phone, $cc);
                if ($user) {
                    $user_id = $user->id;
                }
            }
            ...
            if (empty($user_id)) {
                $this->_send_response(false, null, 'User not found. Please login first.');
                return;
            }

            if (empty($fullname)) {
                $this->_send_response(false, null, 'Full name is required');
                return;
            }
    ...
    {
        try {
            // Accept user_id from query param or session
            $user_id = $this->input->get('user_id') ?: $this->session->userdata('user_id');
            if (empty($user_id)) {
                $this->_send_response(false, null, 'user_id is required');
                return;
            }
    ...
    public function delete_account()
    {
        try {
            $input   = $this->_get_input();
            $user_id = isset($input['user_id']) ? $input['user_id'] : $this->input->post('user_id');
            if (empty($user_id)) {
                $user_id = $this->session->userdata('user_id');
            }
            if (empty($user_id)) {
            ...
            $this->User_model->update($user_id, array('isactive' => 'inactive'));
    ...
    public function refresh_session()
    {
        try {
            ...
            if (!$user || $user->isactive != 'active') {
                $this->session->sess_destroy();
                $this->_send_response(false, null, 'User account is inactive');
                return;
            }
     ...
     * GET /api/mobile/notifications?status=active&limit=10&offset=0
     */
    public function notifications()
    {
        try {
            ...
            $this->_send_response(true, array(
                ...
                'total' => count($this->Notification_model->get_all($status)),
                'limit' => $limit,
                'offset' => $offset
    ...
    public function notification($id = null)
    {
        try {
            ...
            $this->_send_response(true, $this->_format_notification($notification), 'Notification retrieved successfully');
        } catch (Exception $e) {
    ...
    /**
     * Add property to wishlist (toggle — adds if not present, removes if already present)
     * POST /api/mobile/wishlist/store
     * Body: { "user_id": "user_abc123", "property_id": 1 }
     */
    public function wishlist_store()
    {
        try {
            $input       = $this->_get_input();
            $user_id     = isset($input['user_id'])     ? $input['user_id']     : $this->input->post('user_id');
            $property_id = isset($input['property_id']) ? $input['property_id'] : $this->input->post('property_id');

            if (empty($user_id) || empty($property_id)) {
                $this->_send_response(false, null, 'user_id and property_id are required');
                return;
            }
            ...
            if ($this->Wishlist_model->is_wishlisted($user_id, $property_id)) {
                $this->Wishlist_model->remove_from_wishlist($user_id, $property_id);
                $this->_send_response(true, array('wishlisted' => false), 'Removed from wishlist');
                return;
            }

            // Fetch property snapshot for storage
            $property = $this->Property_model->get_by_id($property_id);

            $data = array(
                'user_id'           => $user_id,
                'property_id'       => $property_id,
                'property_name'     => $property ? $property->name      : null,
                'property_image'    => $property ? ($property->main_image ?? null) : null,
                'property_price'    => $property ? $property->price      : null,
                'property_location' => $property ? ($property->location  ?? null) : null
            );

            $id = $this->Wishlist_model->create($data);

            if ($id) {
                $this->_send_response(true, array('wishlisted' => true, 'id' => $id), 'Added to wishlist');
            } else {
                $this->_send_response(false, null, 'Failed to add to wishlist');
            }

        } catch (Exception $e) {
            $this->_send_response(false, null, 'Error: ' . $e->getMessage());
        }
    }
    ...
    public function wishlist_check()
    {
        try {
            ...
            $wishlisted = $this->Wishlist_model->is_wishlisted($user_id, (int)$property_id);

            $this->_send_response(true, array('wishlisted' => $wishlisted), 'Wishlist status checked');
    ...
    public function wishlist_list()
    {
        try {
            ...
            $items = $this->Wishlist_model->get_by_user($user_id, $limit, $offset);
            ...
            foreach ($items as $item) {
                $formatted[] = array(
                    ...
                    'property_image'    => !empty($item->property_image) ? base_url($item->property_image) : null,
                    'property_price'    => $item->property_price ? (float)$item->property_price : null,
    ...
    /**
     * Remove a property from wishlist directly (no toggle)
    ...
    public function wishlist_remove()
    {
        try {
            ...
            $this->Wishlist_model->remove_from_wishlist($user_id, (int)$property_id);
            $this->_send_response(true, array('wishlisted' => false), 'Removed from wishlist');
    ...
    /**
     * Apply a referral code during signup / after signup
     * POST /api/mobile/referral/apply
     * Body: { "user_id": "user_abc123", "referral_code": "REF001A2B" }
     *
     * Finds the owner of the referral code and creates a referral record.
     */
    public function referral_apply()
    {
        try {
            $input         = $this->_get_input();
            $user_id       = isset($input['user_id'])       ? $input['user_id']       : $this->input->post('user_id');
            $referral_code = isset($input['referral_code']) ? $input['referral_code'] : $this->input->post('referral_code');

            if (empty($user_id) || empty($referral_code)) {
                $this->_send_response(false, null, 'user_id and referral_code are required');
                return;
            }

            $referral_code = strtoupper(trim($referral_code));

            // Find the referrer by their referral code in the users table
            $this->db->where('referralcode', $referral_code);
            $referrer = $this->db->get('users')->row();

            if (!$referrer) {
                $this->_send_response(false, null, 'Invalid referral code');
                return;
            }

            // Cannot refer yourself
            if ($referrer->id === $user_id) {
                $this->_send_response(false, null, 'You cannot use your own referral code');
                return;
            }

            // Check if this referral pair already exists
            $this->db->where('referrer_id', $referrer->id);
            $this->db->where('referred_id', $user_id);
            if ($this->db->count_all_results('referrals') > 0) {
                $this->_send_response(false, null, 'Referral code already applied');
                return;
            }
            ...
            $id = $this->Referral_model->create($data);
            ...
            if ($id) {
                ...
                $this->_send_response(false, null, 'Failed to apply referral code');
            }
    ...
    public function referral_list()
    {
        try {
            ...
            $referrals = $this->Referral_model->get_by_referrer($user_id);
            ...
            foreach ($referrals as $ref) {
                $formatted[] = array(
                    ...
                    'status'        => $ref->status,
                    'reward_points' => (int)$ref->reward_points,
    ...
    public function referral_stats()
    {
        try {
            ...
            $user  = $this->User_model->get_by_id($user_id);
            $stats = $this->Referral_model->get_user_stats($user_id);

            $this->_send_response(true, array(
                'referral_code'        => $user ? $user->referralcode : null,

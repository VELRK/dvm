...
(function() {
  function initMainSlider() {
    ...
    if (typeof Swiper === 'undefined') {
      console.error('Swiper is not loaded. Please check script loading order.');
      return false;
    }
    
    if ($(".thumbs-swiper-column").length > 0 && $(".thumbs-swiper-column1").length > 0) {
      // Check if already initialized
      if (!$(".thumbs-swiper-column").hasClass('swiper-initialized')) {
        try {
  ...
  // Try to initialize immediately (scripts are at end of body, DOM should be ready)
  if (!initMainSlider()) {
    // If it fails, try again after DOM is ready
...
if ($(".tf-sw-properties").length > 0) {
  var preview = $(".tf-sw-properties").data("preview");
  var tablet = $(".tf-sw-properties").data("tablet");
  var mobile = $(".tf-sw-properties").data("mobile");
  var mobileSm = $(".tf-sw-properties").data("mobile-sm");
  var spacingLg = $(".tf-sw-properties").data("space-lg");
  var spacingMd = $(".tf-sw-properties").data("space-md");
  var spacing = $(".tf-sw-properties").data("space");
  var swiper = new Swiper(".tf-sw-properties", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    autoplay: {
      delay: 3000,
    ...
    loop: true,
    navigation: {
      nextEl: ".tf-sw-properties .swiper-button-next",
      prevEl: ".tf-sw-properties .swiper-button-prev",
    },
    pagination: {
      el: ".sw-pagination-properties",
      clickable: true,
    },
    breakpoints: {
      575: {
...
if ($(".tf-sw-recent-estate").length > 0) {
  var preview = $(".tf-sw-recent-estate").data("preview");
  var tablet = $(".tf-sw-recent-estate").data("tablet");
  var mobile = $(".tf-sw-recent-estate").data("mobile");
  var mobileSm = $(".tf-sw-recent-estate").data("mobile-sm");
  var spacingLg = $(".tf-sw-recent-estate").data("space-lg");
  var spacingMd = $(".tf-sw-recent-estate").data("space-md");
  var spacing = $(".tf-sw-recent-estate").data("space");
  
  // Count slides to determine if loop should be enabled
  var slideCount = $(".tf-sw-recent-estate .swiper-slide").length;
  var enableLoop = slideCount > 3; // Only enable loop if we have more than 3 slides
  
  var swiper = new Swiper(".tf-sw-recent-estate", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    autoplay: {
      delay: 3000,
    ...
    loop: enableLoop,
    navigation: {
      nextEl: ".tf-sw-recent-estate .swiper-button-next",
      prevEl: ".tf-sw-recent-estate .swiper-button-prev",
    },
    pagination: {
      el: ".sw-pagination-recent-estate",
      clickable: true,
      type: 'bullets',
    },
    breakpoints: {

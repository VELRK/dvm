...
$(document).ready(function () {
  ...
  if (rangeSlider && typeof noUiSlider !== 'undefined' && typeof wNumb !== 'undefined') {
    $(".noUi-handle").on("click", function () {
      $(this).width(50);
    });
    
    // Format function for rupees (lakhs/crores)
    var formatRupees = wNumb({
      decimals: 0,
      thousand: ",",
      prefix: "₹",
    });
    
    // Format display function for lakhs/crores
    ...
    noUiSlider.create(rangeSlider, {
      start: [100000, 50000000], // 1 lakh to 5 crore
      step: 10000, // Step by 10,000
      range: {
        min: [100000], // 1 lakh
    ...
    rangeSlider.noUiSlider.on("update", function (values, handle) {
      ...
      var minValueEl = document.getElementsByName("min-value")[0];
      var maxValueEl = document.getElementsByName("max-value")[0];
      
      // Get numeric values
      var minVal = formatRupees.from(values[0]);
      var maxVal = formatRupees.from(values[1]);
      
      if (value1El) value1El.innerHTML = formatDisplay(minVal);
      if (value2El) value2El.innerHTML = formatDisplay(maxVal);
      if (minValueEl) minValueEl.value = minVal;
      if (maxValueEl) maxValueEl.value = maxVal;
    });
  }
});
...
$(document).ready(function () {
  ...
  if (rangeSlider && typeof noUiSlider !== 'undefined' && typeof wNumb !== 'undefined') {
    ...
    // Set visual min and max values and also update value hidden form inputs
    rangeSlider.noUiSlider.on("update", function (values, handle) {
      var value01El = document.getElementById("slider-range-value01");
      var value02El = document.getElementById("slider-range-value02");
      var minValue2El = document.getElementsByName("min-value2")[0];
      var maxValue2El = document.getElementsByName("max-value2")[0];
      
      if (value01El) value01El.innerHTML = values[0];
      if (value02El) value02El.innerHTML = values[1];
      if (minValue2El) minValue2El.value = moneyFormat.from(values[0]);
      if (maxValue2El) maxValue2El.value = moneyFormat.from(values[1]);
    });
  }
});
